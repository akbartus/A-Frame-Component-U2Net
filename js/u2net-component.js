...
AFRAME.registerComponent('u2net', {
  ...
  init: function () {
    ...
    this.el.addEventListener('loaded', () => {
      ...
      maskCanvas.setAttribute('width', '500');
      maskCanvas.setAttribute('height', '500');
      maskCanvas.style.display = 'none';
      this.el.appendChild(maskCanvas);

      captureButton = document.createElement('button');
      captureButton.setAttribute('id', 'capture');
      captureButton.setAttribute('style', 'position: absolute; bottom: 50px; left: 50%; transform: translateX(-50%); z-index: 3;');
    ...
    this.el.addEventListener('click', (event) => {
      if (event.target === captureButton) {
        ...
        document.querySelector("#selected-image").setAttribute("src", canvas.toDataURL());
        doPrediction();
      }
    });
    ...
    async function doPrediction() {
      ...
      const session = await ort.InferenceSession.create('models/custom_u2netp.onnx', { executionProviders: ['wasm'] }).then(console.log("model loaded"));
      ...
      let input_imageData = octx.getImageData(0, 0, 320, 320); // change this for input
      let floatArr = new Float32Array(320 * 320 * 3)
      let floatArr1 = new Float32Array(320 * 320 * 3)
      let floatArr2 = new Float32Array(320 * 320 * 3)
      ...
      let myImageData = ctx.createImageData(320, 320);
      ...
      resultImg.setAttribute("width", "320");
      resultImg.setAttribute("height", "320");
      document.querySelector("#container").setAttribute("visible", "true");
